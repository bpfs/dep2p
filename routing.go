...
// RoutingDiscovery 是使用 ContentRouting 的发现实现。
// 使用 SHA256 哈希将命名空间转换为 Cids。
...
func (d *RoutingDiscovery) Advertise(ctx context.Context, ns string, opts ...discovery.Option) (time.Duration, error) {
	...
	if ttl == 0 || ttl > 3*time.Hour {
		// DHT提供者记录有效期为24小时，但建议至少每6小时重新发布一次，我们更进一步，每3小时重新发布一次
		ttl = 3 * time.Hour
	}
	...
	// 此上下文需要超时； 它决定 DHT 在继续向密钥/CID 提供记录之前寻找最接近的对等点的时间。
	// 这里不设置超时会让DHT永远徘徊。
...
func (d *RoutingDiscovery) FindPeers(ctx context.Context, ns string, opts ...discovery.Option) (<-chan peer.AddrInfo, error) {
	...
	limit := options.Limit
	if limit == 0 {
		limit = 100 // 这只是任意的，但 FindProvidersAsync 需要一个计数
	}
